{%
laika.title = "Naïve Text Parsing in Scala"
laika.metadata.date = "2008-05-26"
%}
...
Our parser will have to consume the entire format string, including non-escapes.  The final representation will be an immutable list of `Token`(s), either `CharToken` for a single run-of-the-mill character, or `FormatToken` which will represent the fully-parsed substitution.  Thus, for the `printf` example [given above](<#the-printf-case-study>) (my favorite number), the token stream will look something like this:
