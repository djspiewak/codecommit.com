...
categories:
- Scala
date: '2008-04-09 00:00:51 '
layout: post
title: Universal Type Inference is a Bad Thing
wordpress_id: 219
wordpress_path: /scala/universal-type-inference-is-a-bad-thing
---
...
```ml fun sum nil init = init | sum (hd::tail) init = hd + (sum tail init) ``` 
...
```scala def sum(list:List[Int])(init:Int):Int = list match { case hd::tail => hd + sum(tail)(init) case Nil => init } ``` 
...
```scala def sum(list:List[Int])(init:Int):Int = list match { case hd::tail => hd :: sum(tail)(init) // error case Nil => init } ``` 
