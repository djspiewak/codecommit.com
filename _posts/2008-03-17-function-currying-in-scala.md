...
```scala def add(x:Int, y:Int) = x + y add(1, 2) // 3 add(7, 3) // 10 ``` 
...
```scala def add(x:Int, y:Int) = x + y val addCurried = Function.curried(add _) add(1, 2) // 3 addCurried(1)(2) // 3 ``` 
...
```scala def add(x:Int)(y:Int) = x + y val addUncurried = Function.uncurried(add _) add(3)(4) // 7 addUncurried(3, 4) // 7 ``` 
...
```scala def process[A](filter:A=>Boolean)(list:List[A]):List[A] = { lazy val recurse = process(filter) _ list match { case head::tail => if (filter(head)) { head::recurse(tail) } else { recurse(tail) } case Nil => Nil } } val even = (a:Int) => a % 2 == 0 val numbersAsc = 1::2::3::4::5::Nil val numbersDesc = 5::4::3::2::1::Nil process(even)(numbersAsc) // [2, 4] process(even)(numbersDesc) // [4, 2] ``` 
...
``` scala> process(even) _ res0: (List[Int]) => List[Int] =  ``` 
...
```scala //... val even = (a:Int) => a % 2 == 0 val processEvens = process(even) _ val numbersAsc = 1::2::3::4::5::Nil val numbersDesc = 5::4::3::2::1::Nil processEvens(numbersAsc) // [2, 4] processEvens(numbersDesc) // [4, 2] ``` 
...
```scala def add(x:Int, y:Int, z:Int) = x + y + z val addFive = add(5, _:Int, _:Int) addFive(3, 1) // 9 ``` 
...
```scala def add(x:Int, y:Int, z:Int) = x + y + z val addFive = (a:Int, b:Int) => add(5, a, b) addFive(3, 1) // 9 ``` 
