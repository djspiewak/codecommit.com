...
```java String[] names = { "Daniel", "Chris", "Joseph" }; for (String name : names) { System.out.println(name); } ``` 
...
```ruby names = ['Daniel', 'Chris', 'Joseph'] names.each do |name| puts name end ``` 
...
```scala val names = Array("Daniel", "Chris", "Joseph") names.foreach { name => println(name) } ``` 
...
```scala val names = Array("Daniel", "Chris", "Joseph") names.foreach(println) ``` 
...
```scala val nums = List(1, 2, 3, 4, 5) var sum = 0 for (n <\- nums) { sum += n } ``` 
...
```scala val nums = List(1, 2, 3, 4, 5) val sum = nums.foldLeft(0) { (total, n) => total + n } ``` 
...
```scala val nums = List(1, 2, 3, 4, 5) (0 /: nums) {_+_} ``` 
...
```scala val names = List("Daniel", "Chris", "Joseph") val str = names.foldLeft("") { (acc, n) => acc + ", " + n } println(str) ``` 
...
```scala val names = List("Daniel", "Chris", "Joseph") val str = names.reduceLeft[String] { (acc, n) => acc + ", " + n } println(str) ``` 
...
```scala val strs = List("1", "2", "3", "4", "5") val nums = strs.map { s => s.toInt } nums == List(1, 2, 3, 4, 5) // true ``` 
...
```java public void rpcMethod(Object[] params) { String[] strParams = new String[params.length]; for (int i = 0; i < params.length; i++) { strParams[i] = (String) params[i]; } } ``` 
...
```scala def rpcMethod(params: Array[Object]) { val strParams = params.map { _.asInstanceOf[String] } } ``` 
...
```scala def rpcMethod(params: Array[Object]) { val strParams = for (p <\- params) yield p.asInstanceOf[String] } ``` 
...
```scala class List[A] { // don't try this at home def flatMap[B](f: (A)=>List[B]): List[B] } ``` 
...
```scala val strs = List("1", "two", "3", "four", "five") val nums = strs.flatMap { s => try { List(s.toInt) } catch { case _ => Nil } } nums == List(1, 3) // true ``` 
