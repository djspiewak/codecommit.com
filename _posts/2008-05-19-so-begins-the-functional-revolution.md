...
categories:
- Scala
date: '2008-05-19 00:00:21 '
layout: post
title: So Begins the Functional Revolution
wordpress_id: 229
wordpress_path: /scala/so-begins-the-functional-revolution
---
...
```ml fun search nil _ = false | search (hd::tail) x = (hd = x) orelse (search tail x) ``` 
...
```java public  boolean search(List list, T element) { for (T hd : list) { if (element.equals(hd)) { return true; } } return false; } ``` 
...
```scala object Main { def main(args: Array[String]) { println(args.map(_.toInt).foldLeft(0)(_ + _)) } } ``` 

Compare that to the equivalent Java code:

```java public class Main { public static void main(String[] args) { int sum = 0; for (String arg : args) { sum += Integer.parseInt(arg); } System.out.println(sum); } } ``` 
