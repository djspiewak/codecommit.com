...
categories:
- Java
date: '2007-08-13 17:30:13 '
layout: post
title: 'Even More ActiveObjects: Preloading'
wordpress_id: 120
wordpress_path: /java/even-more-activeobjects-preloading
---
...
```java for (Person p : manager.find(Person.class)) { System.out.println(p.getName()); } ``` 

This will generate the following SQL (assuming 6 rows in the people table):

```sql SELECT id FROM people SELECT name FROM people WHERE id = ? SELECT name FROM people WHERE id = ? SELECT name FROM people WHERE id = ? SELECT name FROM people WHERE id = ? SELECT name FROM people WHERE id = ? SELECT name FROM people WHERE id = ? ``` 
...
```java Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement("SELECT name FROM people"); ResultSet res = ps.executeQuery(); while (res.next()) { System.out.println(res.getString("name")); } res.close(); ps.close(); conn.close(); ``` 
...
```java for (Person p : manager.find(Person.class, Query.select("id,name"))) { System.out.println(p.getName()); } ``` 
...
```java @Preload("name") public interface Person extends Entity { public String getName(); public void setName(String name); public int getAge(); public void setAge(int age); } // ... for (Person p : manager.find(Person.class)) { System.out.println(p.getName()); } ``` 
...
```java @Preload({"name", "age"}) public interface Person extends Entity { // ... } ``` 

Or, since this is really all of the properties in Person, we can use the following, shorter syntax:

```java @Preload public interface Person extends Entity { // ... } ``` 
