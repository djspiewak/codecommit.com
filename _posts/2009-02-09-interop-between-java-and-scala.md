...
layout: post
title: Interop Between Java and Scala
wordpress_id: 284
wordpress_path: /java/interop-between-java-and-scala
---
...
```java public class Person { public String getName() { return "Daniel Spiewak"; } } ``` 

...and the second in Scala:

```scala class Person { def getName() = "Daniel Spiewak" } ``` 
...
```scala val p = new Person() p.getName() // => "Daniel Spiewak" ``` 

...and from Java:

```java Person p = new Person(); p.getName(); // => "Daniel Spiewak" ``` 
...
```scala class MyAbstractButton extends JComponent { private var pushed = false def setPushed(p: Boolean) { pushed = p } def getPushed = pushed override def paintComponent(g: Graphics) { super.paintComponent(g) // draw a button } } ``` ```java public class ProKitButton extends MyAbstractButton { // do something uniquely Apple-esque } ``` 
...
```scala trait Model { def value: Any } ``` 
...
```java public interface Model { public Object value(); } ``` 

Thus, we can declare traits in Scala and implement them as interfaces in Java classes:

```java public class StringModel implements Model { public Object value() { return "Hello, World!"; } } ``` 

This is precisely equivalent to a Scala class which mixes-in the `Model` trait:

```scala class StringModel extends Model { def value = "Hello, World!" } ``` 
...
```scala trait Model { def value: Any def printValue() { println(value) } } ``` 
...
```java public interface Model extends ScalaObject { public Object value(); public void printValue(); } public class Model$class { public static void printValue(Model self) { System.out.println(self.value()); } } ``` 
...
```java public class StringModel implements Model { public Object value() { return "Hello, World!"; } public void printValue() { Model$class.printValue(this); } // method missing here (see below) } ``` 
...
```java public class StringModel implements Model { public Object value() { return "Hello, World!"; } public void printValue() { Model$class.printValue(this); } public int $tag() { return 0; } } ``` 
...
```java public abstract class List { ... } ``` 
...
```java import scala.Tuple2; ... Tuple2 me = new Tuple2("Daniel", "Spiewak"); ``` 
...
```scala abstract class List[+A] { def ::[B >: A](e: B) = ... def +[B >: A](e: B) = ... } ``` 
...
```java public abstract class List { public **List **$colon$colon(B e) { ... } public **List **$plus(B e) { ... } } ```
...
```scala import scala.reflect.BeanProperty class Person { @BeanProperty var name = "Daniel Spiewak" } ``` 
...
```java // *without* @BeanProperty public class Person { private String name = "Daniel Spiewak"; public String name() { return name; } public void name_$eq(String name) { this.name = name; } } ``` 
...
```java public class Person { private String name = "Daniel Spiewak"; public String name() { return name; } public String getName() { return name(); } public void name_$eq(String name) { this.name = name; } public void setName(String name) { name_$eq(name); } } ``` 
