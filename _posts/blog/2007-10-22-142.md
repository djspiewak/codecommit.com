...
- Scripts
- Linux
date: '2007-10-22 00:00:37 '
layout: post
title: How to Reimage Your Drive and Lose Your OS
wordpress_id: 142
wordpress_path: /scripts/142
---
...
```bash cat file.txt | dd > file.txt # how useless can we get? ``` 
...
```bash dd if=/dev/sda of=my-drive.img # image the first sata/scsi drive into "my-drive.img" ``` 
...
```bash dd if=/dev/sda | gzip > image-`date +%m%d%y`.img.gz ``` 
...
```bash zcat image-*.img.gz | dd of=/dev/sda ``` 
