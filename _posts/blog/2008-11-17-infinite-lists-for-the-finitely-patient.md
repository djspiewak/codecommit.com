...
```scala class RichStream[A](str: =>Stream[A]) { def ::(hd: A) = Stream.cons(hd, str) } implicit def streamToRichStream[A](str: =>Stream[A]) = new RichStream(str) ``` 
...
```scala def from(n: Int): Stream[Int] = n :: from(n + 1) val nats = from(0) ``` 
...
```scala def from(n: Int): Stream[Int] = { println("Requesting n = " + n) n :: from(n + 1) } val nats = from(0) ``` 
...
```scala nats(4) // get the fifth natural number (4) ``` 

This will print the following:

``` Requesting n = 1 Requesting n = 2 Requesting n = 3 Requesting n = 4 ``` 
...
```scala val nats = from(0) val fives = nats map { _ * 5 } println("Querying list...") fives(4) // get the fifth multiple of five (20) ``` 

When this code executes, it will print the following:

``` Requesting n = 0 Querying list... Requesting n = 1 Requesting n = 2 Requesting n = 3 Requesting n = 4 ``` 
...
```scala for (n <\- fives) { println(n) } ``` 

If we were to execute this snippet, the result would be as follows:

``` Requesting n = 0 0 Requesting n = 1 5 Requesting n = 2 10 Requesting n = 3 15 Requesting n = 4 20 Requesting n = 5 25 Requesting n = 6 30 . . . ``` 
...
```scala val nats = from(0) val digits = nats.take(10) for (d <\- digits) { println(d) } ``` 
...
``` Requesting n = 0 0 Requesting n = 1 1 Requesting n = 2 2 Requesting n = 3 3 Requesting n = 4 4 Requesting n = 5 5 Requesting n = 6 6 Requesting n = 7 7 Requesting n = 8 8 Requesting n = 9 9 ``` 
...
```cpp char *names[6] = { "Daniel", "Chris", "Joseph", "Renee", "Bethany", "Grace" }; int len = 6; for (int i = 0; i < len; ++i) { cout << names[i] << endl; } ``` 
...
```scala val names = Array("Daniel", "Chris", "Joseph", "Renee", "Bethany", "Grace") def loop(i: Int) { if (i < names.length) { println(names(i)) loop(i + 1) } } loop(0) ``` 
...
```scala for (i <\- nats.take(names.length)) { println(names(i)) } ``` 
...
```scala for (i <\- 0 until names.length) { println(names(i)) } ``` 
...
```java int a = 0; for (int i1 = 0; i1 < 10; i1++) { a = // generate random number int b = 0; for (int i2 = 0; i2 < 10; i2++) { b = // generate random number System.out.println("(" + a + ", " + b + ")"); } } ``` 
...
```scala def randomGen: Stream[Int] = (Math.random * 200).toInt - 100 :: randomGen val random = randomGen for (a <\- random.take(10); b <\- random.take(10)) { println("(" + a + ", " + b + ")") } ``` 

When this executes, it will print the following sequence:

``` (68, 68) (68, -83) (68, -79) (68, -78) (68, -18) (68, -80) (68, 29) (68, 10) (68, -63) (68, 3) (-83, 68) (-83, -83) (-83, -79) (-83, -78) . . . ``` 
...
```scala def random: Stream[Int] = (Math.random * 200).toInt - 100 :: random // or even... val random: Stream[Int] = (Math.random * 200).toInt - 100 :: random ``` 
...
```scala def ancestors(id: Int) = { val conn: Connection = ... try { val res = conn.executeQuery("SELECT parentID FROM people WHERE id = ?") if (res.next()) { val parentID = res.getInt(1) parentID :: ancestors(parentID) } else Stream.empty } finally { conn.close() } } ``` 
...
```scala val parents = ancestors(42) val fewerParents = parents filter { _ % 2 == 1 } for (id <\- fewerParents) { println(nameForPerson(id)) // get a person's name and print } ``` 
