...
categories:
- Scala
date: '2010-05-24 02:00:36 '
layout: post
title: Working with Scala's XML Support
wordpress_id: 312
wordpress_path: /scala/working-with-scalas-xml-support
---
...
```scala val nodes: Seq[Node] = ... val ns: NodeSeq = NodeSeq fromSeq nodes ``` 
...
```scala val ns = **Hello,** World! println(ns.toString) // prints the raw XML ``` 
...
```scala val ns = Elem(null, "span", new UnprefixedAttribute("id", Text("foo"), Null), TopScope, Elem(null, "strong", Null, TopScope, Text("Hello,")), Text(" World!")) println(ns.toString) ``` 
...
```scala new UnprefixedAttribute("id", , Null) ``` 
...
```scala def makeXML(id: String) = **Hello,** World! makeXML("foo") // => ... ``` 
...
```scala val ns = Text ns \ "foo" // => ... ns \ "foo" \ "bar" // => Text ``` 
...
```scala val ns = Text ns \\\ "bar" // => Test ns \\\ "baz" // =>  ``` 
...
```scala ns \\\ "@id" find { _.text == "1" } // => Some("1") ``` 
...
```scala  match { // prints "foo" case  => println("foo") case  => println("bar") } ``` 
...
```scala bar match { // throws a MatchError! case  => println("foo") case  => println("bar") } ``` 
...
```scala bar match { // prints "foo" case bar => println("foo") case bar => println("bar") } ``` 
...
```scala mystery match { // prints "foo: mystery" case { txt } => println("foo: " + txt) case { txt } => println("bar: " + txt) } ``` 
...
```scala mystery match { // throws a MatchError! case { txt } => println("foo: " + txt) case { txt } => println("bar: " + txt) } ``` 
...
```scala mystery match { // prints "foo: ArrayBuffer(mystery,)" case { ns @ _* } => println("foo: " + ns) case { ns @ _* } => println("bar: " + ns) } ``` 
...
```scala mystery match { // prints "foo: mystery" case { ns @ _* } => println("foo: " + NodeSeq.fromSeq(ns)) case { ns @ _* } => println("bar: " + NodeSeq.fromSeq(ns)) } ``` 
...
```scala  match { case  => println("bar") // does not compile! case  => println("baz") } ``` 
...
```scala  match { // prints "bar" case n @  if (n \ "@id" text) == "bar" => println("bar") case n @  if (n \ "@id" text) == "baz" => println("baz") } ``` 
...
```scala  match { // prints "gadget" case  => println("gadget") } ``` 

If you want to match a specific namespace, you need to include it in the pattern:

```scala  match { // prints "w:gadget" case  => println("m:gadget") case  => println("w:gadget") } ``` 
