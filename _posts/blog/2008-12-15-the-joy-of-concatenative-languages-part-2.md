...
categories:
- Cat
date: '2008-12-15 02:00:53 '
layout: post
title: 'The Joy of Concatenative Languages Part 2: Innately Functional'
wordpress_id: 276
wordpress_path: /cat/the-joy-of-concatenative-languages-part-2
---
...
```cat define plus { + } define minus { - } 7 2 3 plus minus ``` 
...
```scala def plus(a: Int, b: Int) = a + b def minus(a: Int, b: Int) = a - b minus(7, plus(2, 3)) ``` 
...
```cat 5 0 < [ "strange math" ] [ "all is well" ] if ``` 
...
```cat define append { empty [ pop ] [ uncons [append] dip cons ] if } ``` 
...
```ml fun append ls nil = ls | append ls (hd :: tail) = hd :: (append ls tail) ``` 
...
```cat define cat { swap [cons] rfold } ``` 
...
```ml val length = foldl (fn (n, _) => n + 1) 0 ``` 
