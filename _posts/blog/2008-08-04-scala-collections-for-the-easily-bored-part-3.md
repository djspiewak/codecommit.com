...
layout: post
title: 'Scala Collections for the Easily Bored Part 3: All at Once'
wordpress_id: 251
wordpress_path: /scala/scala-collections-for-the-easily-bored-part-3
---
...
```sql SELECT * FROM people WHERE age > 18 AND affiliation = 'Green Party' ``` 
...
```scala def onlyEven(nums: List[Int]) = { nums.filter((n) => n % 2 == 0) } ``` 
...
```scala def onlyEven(nums: List[Int]) = { for { n <\- nums if n % 2 == 0 } yield n } ``` 
...
```scala def separateEven(nums: List[Int]) = { nums.partition((n) => n % 2 == 0) } ``` 
...
```scala val numbers = List(1, 2, 3, 4, 5, 6, 7) val sep = separateEven(numbers) sep == (List(2, 4, 6), List(1, 3, 5, 7)) // true ``` 
...
```scala val cache = new HashMap[Int, String] def readData(id: Int) = { if (cache.contains(id)) { // gonna find her cache(id) } else { val conn = createConnection() val back = try { val stmt = conn.prepareStatement("SELECT value FROM dataTab WHERE id = ?") stmt.setInt(1, id) val res = stmt.executeQuery() if (res.next()) { res.getString(1) } else { null } } finally { conn.close() } cache += (id -> back) back } } ``` 
...
```scala val nums = List(1, 2, 3, 4, 5, 6) nums.exists((n) => (3 to 5).contains(n)) ``` 
...
```scala val nums = List(1, 2, 3, 4, 5, 6) val elem = nums.find((n) => (3 to 5).contains(n)) elem == Some(3) // true ``` 
