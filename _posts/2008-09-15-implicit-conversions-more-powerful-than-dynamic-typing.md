...
layout: post
title: 'Implicit Conversions: More Powerful than Dynamic Typing?'
wordpress_id: 260
wordpress_path: /ruby/implicit-conversions-more-powerful-than-dynamic-typing
---
...
```scala val acronym = "Microsoft Certified Systems Engineer".reduce println(acronym) // MCSE ``` 
...
```scala class MyRichString(str: String) { def reduce = str.toCharArray.foldLeft("") { (t, c) => t + (if (c.isUpperCase) c.toString else "") } } implicit def str2MyRichString(str: String) = new MyRichString(str) ``` 
...
```ruby class String def reduce arr = unpack('c*').select { |c| (65..90).include? c } arr.pack 'c*' end end puts 'HyperText Transfer Protocol'.reduce # HTTP ``` 
...
```scala implicit def lessThanOverload(i: Int) = new { def <(str: String) = str.length < i } ``` 
...
```scala implicit def str2Int(str: String) = str.length ``` 
...
```cpp bool operator<(const int &i;, const std::string &str;) { return str.length() < i; } ``` 
...
```ruby class Fixnum def <(str) str.size < self end end ``` 
...
```ruby class Fixnum def <(str) self >= str.size end end ``` 
...
```ruby class Fixnum alias_method :__old_less_than__, '<'.to_sym def <(target) if target.kind_of? String __old_less_than__ target.size else __old_less_than__ target end end end ``` 
