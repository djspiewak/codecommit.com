...
categories:
- Scala
date: '2008-04-07 00:00:35 '
layout: post
title: The "Option" Pattern
wordpress_id: 218
wordpress_path: /scala/the-option-pattern
---
...
```scala def div(a:Int)(b:Int):Option[Int] = if (b <= 0) None else if (a < b) Some(0) else Some(1 + div(a - b)(b).get) ``` 
...
```scala div(25)(5) // => Some(5) div(150)(2) // => Some(75) div(13)(4) // => Some(3) ``` 
...
```scala div(13)(0) // => None div(25)(-5) // => None ``` 
...
```scala div(13)(0) match { case Some(x) => println(x) case None => println("Problems") } // => prints "Problems" div(25)(5) match { case Some(x) => println(x) case None => println("Problems") } // => prints "5" ``` 
...
```java public class Example { private String value; public String getValue() { if (value == null) { value = queryDatabase(); } return value; } } // ... Example ex = new Example(); System.out.println(ex.getValue()); ``` 
...
```java public class Example { private int value; public int getValue() { if (value == ???) { // er...? value = queryDatabase(); } return value; } } // ... Example ex = new Example(); System.out.println(ex.getValue()); ``` 
...
```java public class Example { private Integer value = null; public int getValue() { // forgot to init... return value; } } ``` 
...
```java public interface Option { public T get(); } public final class Some implements Option { private final T value; public Some(T value) { this.value = value; } public T get() { return value; } } public final class None implements Option { public None() {} public T get() { throw new UnsupportedOperationException("Cannot resolve value on None"); } } ``` 

The usage for this code looks like this:

```java public class Example { private Option value = new None(); public boolean getValue() { if (value instanceof None) { value = queryDatabase(); } return value.get(); } } ``` 
